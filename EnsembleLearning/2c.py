...
for i in range(100):
    trees = bagged_decision_trees(train_data, attributes, attribute_names, T=500)
    all_bagging_models.append(trees)
    all_single_trees.append(trees[0])
    print(i)

single_tree_predictions = []
bagging_models_predictions = []
...
bagging_models_predictions = [predict_bagging(model, test_data, attribute_indices) for model in all_bagging_models]

for predictions, model_name in [(single_tree_predictions, "Single Tree"), (bagging_models_predictions, "Bagging Models")]:
