...
for gamma in gamma_values:
    for C in C_values:
        ...
        support_vector_counts[(gamma, C)] = len(sv_indices)
...
C_target = 500/873
previous_sv_indices = None

for gamma in gamma_values:
    alpha, b, sv_indices = svm_dual_gaussian(x_train, y_train, C_target, gamma)
    if previous_sv_indices is not None:
        overlap = len(set(previous_sv_indices).intersection(set(sv_indices)))
        overlap_counts[(previous_gamma, gamma)] = overlap
        print(f"Overlap between gamma = {previous_gamma} and gamma = {gamma}: {overlap}")
    previous_sv_indices = sv_indices
